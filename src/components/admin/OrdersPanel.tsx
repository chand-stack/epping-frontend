...
const OrdersPanel: React.FC = () => {
  ...
  const load = () => setOrders(orderManagementService.getOrders().slice().reverse());

  useEffect(() => {
    load();
  }, []);

  const advance = (order: OrderStatus) => {
    const idx = statusSteps.indexOf(order.status);
    const next = statusSteps[Math.min(idx + 1, statusSteps.length - 1)];
    orderManagementService.updateOrderStatus(order.id, next);
    load();
  };
