#!/usr/bin/env python3
"""
Create final combined lead file with proper statistics
"""
...
def create_final_combined_file():
    """Create the final combined lead file"""
    ...
    # Clean up email column - replace empty strings and NaN with empty string
    combined_df['emails'] = combined_df['emails'].fillna('')
    combined_df['emails'] = combined_df['emails'].astype(str)
    combined_df['emails'] = combined_df['emails'].replace('nan', '')
    ...
    output_file = os.path.join(data_dir, f"MASTER_LEADS_WITH_EMAILS_{timestamp}.csv")
    ...
    for query in sorted(combined_df['query_used'].unique()):
    ...
    # Show sample results with emails
    print(f"\n📋 Sample leads with emails:")
    sample_leads = combined_df[combined_df['emails'] != ''].head(10)
    for _, lead in sample_leads.iterrows():
        print(f"   • {lead['name']} ({lead['query_used']}): {lead['emails']}")
    
    # Show some leads without emails
    print(f"\n📋 Sample leads without emails:")
    no_email_leads = combined_df[combined_df['emails'] == ''].head(5)
    for _, lead in no_email_leads.iterrows():
        print(f"   • {lead['name']} ({lead['query_used']}): No email found")
